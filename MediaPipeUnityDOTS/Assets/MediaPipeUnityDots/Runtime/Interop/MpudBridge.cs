...
namespace MediaPipeUnityDots.Runtime.Interop
{
    ...
    /// <summary>
    /// MediaPipe Holistic Tracker 네이티브 브리지 P/Invoke 래퍼.
    /// 모든 메서드는 메인 스레드에서 호출할 것 (thread_local 에러 저장소 주의, 다른 bridge와 별개).
    /// </summary>
    public static class MpudHolisticBridge
    {
        private const string DllName = "mpud_bridge";

        [DllImport(DllName, CallingConvention = CallingConvention.Cdecl)]
        public static extern int mpud_create_holistic_tracker(
            ref MpudHolisticTrackerConfig config, out IntPtr tracker);

        [DllImport(DllName, CallingConvention = CallingConvention.Cdecl)]
        public static extern void mpud_destroy_holistic_tracker(IntPtr tracker);

        [DllImport(DllName, CallingConvention = CallingConvention.Cdecl)]
        public static extern int mpud_submit_holistic_frame(
            IntPtr tracker, ref MpudImageFrame frame);

        [DllImport(DllName, CallingConvention = CallingConvention.Cdecl)]
        public static extern int mpud_try_get_latest_holistic_result(
            IntPtr tracker, out MpudHolisticResult result);

        [DllImport(DllName, CallingConvention = CallingConvention.Cdecl)]
        public static extern IntPtr mpud_get_last_holistic_error();

        public static string GetLastHolisticError()
        {
            var ptr = mpud_get_last_holistic_error();
            return ptr == IntPtr.Zero ? "unknown error" : Marshal.PtrToStringAnsi(ptr);
        }
    }
}
